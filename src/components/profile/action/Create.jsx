...
const Create = ({ onCreate }) => {
  const [item, setItem] = useState({ NamaProfile: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("NamaProfile", item.NamaProfile);
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 w-full flex items-end">
      <div className="w-full">
        <INP
          placeholder="Nama "
          value={item.NamaProfile}
          onChange={(e) => setItem({ ...item, NamaProfile: e.target.value })}
        />
