...
const Create = ({ onCreate }) => {
  const [item, setItem] = useState({ NamaSerti: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("NamaSerti", item.NamaSerti);
    formData.append("Gambar", imageFile);

    try {
      const response = await axios.post(`${apiUrl}/serti`, formData, {
        headers: {
    ...
    <form onSubmit={handleSubmit} className="mb-4 w-full flex items-end">
      <div className="w-full">
        <INP
          ...
          value={item.NamaSerti}
          onChange={(e) => setItem({ ...item, NamaSerti: e.target.value })}
        />
