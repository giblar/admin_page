...
const Project = () => {
  ...
  const handleUpdate = (updatedItem) => {
    setItems(items.map(item => (item.Project_id === updatedItem.Project_id ? updatedItem : item)));
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setItems(items.filter(item => item.Project_id !== id));
    } catch (error) {
  ...
  return (
    <div className='w-full bg-[#1F2937]  min-h-screen max-h-screen overflow-y-auto text-white p-7'>
     <h1 className='text-5xl font-bold uppercase py-10 px-5'>project</h1>
